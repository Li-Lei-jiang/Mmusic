const apiReq = {
	req(param) {
		...
		if (!token) { //本地无token需重新登录
			...
			console.log('没有Token,赶紧滚去登录')
			return;
		} 
		
		// if (!data.token) { //固定参数:如果参数中无token(除了小程序第一次通过code获取token的接口默认参数token = login,其他接口token参数都是在本地缓存中获取)
	...
	var	BaseUrl = 'http://49.233.66.125:3000';
		//拼接完整请求地址
